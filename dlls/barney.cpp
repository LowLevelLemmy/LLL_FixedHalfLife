...
public:
	...
	int VolMulti = 0;	//Voice line gets louder everytime he asks for a passport
	CUSTOM_SCHEDULES;
};
...
Task_t	tlChasePlayer[] =
{
	{ TASK_WAIT,				(float)4		},
	{ TASK_MOVE_TO_TARGET_RANGE,(float)70		},	// Move within 128 of target ent (client)
	{ TASK_SET_SCHEDULE,		(float)SCHED_SLEEP },
};

Schedule_t slChasePlayer[] =
{
	{
		tlChasePlayer,
		ARRAYSIZE(tlChasePlayer),
		0,
		0,
		"ChasePlayer"
	}
...
Schedule_t* CBarney :: GetScheduleOfType ( int Type )
{
	...
	case SCHED_WAKE:
	{
		ALERT(at_console, "RETURNING chaseplayer\n");
		//ALERT(at_console, "Playing Scentense\n");
		//PlaySentence("BA_KILL", 4, VOL_NORM, ATTN_NORM);	//Voice Line regurding pass
		return slChasePlayer;
	}
	case SCHED_SLEEP:
	{
		ALERT(at_console, "Give me your PASSPORT!!\n");
		PlaySentence("BA_KILL", 4, VOL_NORM + VolMulti, ATTN_NORM);	//Voice Line regurding pass
		VolMulti *= 2;
		return slChasePlayer;
	}
...
// of the proper type.
//=========================================================
Schedule_t *CBarney :: GetSchedule ( void )	//chooses the most fitting schedule, then GetScheduleOfType actually gets the scedue u wrote
{
	if (!PassAquired)
	{
		CBaseEntity* player = FindNearestFriend(1);
		if (player)
		{
			m_hTargetEnt = player;
			return GetScheduleOfType(SCHED_WAKE);
		}
	}

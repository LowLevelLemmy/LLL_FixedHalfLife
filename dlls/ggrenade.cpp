...
void CGrenade :: TumbleThink( void )
{
	...
	if (pev->velocity == g_vecZero && !HasStopped)	//Has the grenade stopped moving?
	{
		//ALERT(at_console, "Grenade Has Stopped, playing sound");
		// ok, start stinkin!
		CSoundEnt::InsertSound(bits_SOUND_MEAT, pev->origin, 384, 10);
		HasStopped = true;
	}
}
